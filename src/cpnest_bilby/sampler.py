...
class CPNest(NestedSampler):
    ...
    def run_sampler(self):
        ...
        while out is None:
            try:
                out = CPNestSampler(model, **self.kwargs)
                ...
                logger.info(
                    f"CPNest init. failed with error {e}, please update"
                )
        ...
        self.result.log_evidence_err = np.sqrt(
            out.NS.state.info / out.NS.state.nlive
        )
        ...
        if "proposals" in self.kwargs:
            ...
            for key, proposal in self.kwargs["proposals"].items():
                if isinstance(proposal, JumpProposalCycle):
                    self.kwargs["proposals"][key] = (
                        cpnest_proposal_cycle_factory(proposal)
                    )
...
def cpnest_proposal_factory(jump_proposal):
    ...
    class CPNestEnsembleProposal(cpnest.proposal.EnsembleProposal):
        ...
        def get_sample(self, cpnest_sample, **kwargs):
            ...
            sample = self.jump_proposal(
                sample=sample, sampler_name="cpnest", **kwargs
            )
    ...
    class CPNestProposalCycle(cpnest.proposal.ProposalCycle):
        def __init__(self):
            ...
            for i, prop in enumerate(self.jump_proposals.proposal_functions):
                self.jump_proposals.proposal_functions[i] = (
                    cpnest_proposal_factory(prop)
                )
        ...
        def get_sample(self, old, **kwargs):
            return self.jump_proposals(
                sample=old, coordinates=self.ensemble, **kwargs
            )
